...
/**
 * Gmail-specific operations layered on top of AccountTokenManager.
 *
 * The token cache, refresh, rate-limiting, and 429/5xx retry logic all live in
 * AccountTokenManager so they can be shared with CalendarClient (one access
 * token carries all granted scopes). This class only owns the Gmail API
 * mechanics: simplifying responses, MIME encoding, and the HTTP batch path
 * used by the bulk thread tools.
 *
...
class GmailClient
{
    ...
    public function searchThreads(string $account, string $query, int $pageSize = 25, ?string $pageToken = null): array
    {
        ...
        $response = $this->tokens->call($account, 5, fn () => $service->users_threads->listUsersThreads('me', $params));
    ...
    {
        $service = $this->gmail($account);
        $thread = $this->tokens->call($account, 10, fn () => $service->users_threads->get('me', $threadId, ['format' => 'full']));
    ...
    {
        $service = $this->gmail($account);
        $response = $this->tokens->call($account, 1, fn () => $service->users_labels->listUsersLabels('me'));
    ...
    public function createLabel(string $account, string $name, ?string $backgroundColor = null, ?string $textColor = null): array
    {
        ...
        $created = $this->tokens->call($account, 5, fn () => $service->users_labels->create('me', $label));
    ...
    public function modifyThread(string $account, string $threadId, array $addLabelIds = [], array $removeLabelIds = []): void
    {
        ...
        $this->tokens->call($account, 10, fn () => $service->users_threads->modify('me', $threadId, $request));
    }
    ...
    {
        $client = $this->tokens->authenticatedClient($account);
        ...
        foreach (array_chunk($threadIds, self::BATCH_CHUNK) as $chunk) {
            ...
            try {
                ...
                // Route the batch execute through the token manager so it
                // gets the same rate-limit + 429/5xx retry treatment as a
                // single call. Cost scales with chunk size.
                $responses = $this->tokens->call(
                    $account,
                    count($chunk) * 10,
                    fn () => $batch->execute(),
                );
            ...
            foreach ($chunk as $threadId) {
                ...
                $results[$threadId] = $outcome instanceof Throwable
                    ? ['status' => 'error', 'error' => $outcome->getMessage()]
                    : ['status' => 'ok'];
            }
        }
    ...
    public function createDraft(
        ...
        if ($replyToMessageId !== null && $replyToMessageId !== '') {
            $original = $this->tokens->call($account, 5, fn () => $service->users_messages->get('me', $replyToMessageId, ['format' => 'metadata']));
        ...
        $created = $this->tokens->call($account, 10, fn () => $service->users_drafts->create('me', $draft));
    ...
    {
        return new Gmail($this->tokens->authenticatedClient($account));
    }
